...
class Existent {
    ...
    getNounPhrase(role, spin, ev) {
        ...
        if (spin.narrator === this) {
            switch (role) {
                case "subject":
                    { phrase = this.pronoun.getSubject(1, this.number); break; }
                case "object":
                    { phrase = this.pronoun.getObject(1, this.number); }
            }
        } else if (spin.narratee === this) {
            switch (role) {
                case "subject":
                    { phrase = this.pronoun.getSubject(2, this.number); break; }
                case "object":
                    { phrase = this.pronoun.getObject(2, this.number); }
            }
        } else if (this.owner) {
            ...
            switch (role) {
                case "subject":
                    { phrase = this.pronoun.getSubject(3, this.number); break; }
                case "object":
                    { phrase = this.pronoun.getObject(3, this.number); }
            }
        } else if (!this.article) {
            phrase = this.name;
        } else if (givens.has(this) && ["a", "an", "one", "several", "some"].includes(this.article)) {
...
class PronounSet {
    constructor(thirdPersonSingular) {
        ...
        this.pronoun.push([
            [], thirdPersonSingular, ["they", "them", "their"]
        ]);
    }
...
class Event {
    ...
    setTemplate(custom = null) {
        ...
        if (custom != null) {
            ...
            if (this.object) {
                if (typeof this.object === "string") { this.template += " " + this.object; } else { this.template += " [object/o]"; }
            }
    ...
    placeVerbPhrase(currentTemplate, spin, agent) {
        ...
        switch (spin.speaking) {
            case "after":
                { tenseER = "past"; break; }
            case "during":
                { tenseER = "present"; break; }
            case "before":
                { tenseER = "future"; break; }
        }
    ...
    for (var i of telling) {
        ...
        if (typeof lastNarratedEvent !== "undefined" && event.start < lastNarratedEvent.start) {
            ...
            if (spin.speaking === "after") {
                if (spin.referring === "posterior") { spin.referring = "simple"; } else if (spin.referring === "simple") { spin.referring = "anterior"; }
            }
...
function getParameters(actor) {
    ...
    if (params.substring(0, 1) === "?") {
        ...
        for (var p of params) {
            pair = p.split("=");
            if (pair[0] === "narrator" || pair[0] === "narratee") { spin[pair[0]] = actor[pair[1]]; } else if (pair[0] === "time_markers") { spin.time_markers = true; } else if (pair[0] === "event_numbers") { spin.event_numbers = true; } else if (pair[0] === "expression_numbers") { spin.expression_numbers = true; } else { spin[pair[0]] = pair[1]; }
        }
    }
